...
def x_coordinate_function(node):
    """
    Equivalent to anonymous function lambda k: k.height - 0.05
    """
    return node.height - 0.05

ll.plot_text(ax, xCoordinateFxn = x_coordinate_function, targetFxn = target_function, textContentFxn = text_function, 
             horizontalalignment = 'right', verticalalignment = 'bottom', size = 12)

ll.plot_points(ax, targetFxn = target_function)

bt_utils.clean_axes(ax)

plt.savefig('basic-tree-text-2.png', bbox_inches='tight')