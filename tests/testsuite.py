...
class test_parsers(unittest.TestCase):
    ...
    def test_newick(self):
        ...
        assert len(tree.Objects) == expected_num_nodes, (
            f"Newick tree does not contain correct number of nodes. Expected: {expected_num_nodes}. Observed: {len(tree.Objects)}"
        )
        max_height = round(max([i.height for i in tree.Objects]), 4)
        expected_height = 0.0058
        assert max_height == expected_height, (
            f"Newick tree height is not correct. Expected: {expected_height}. Observed: {max_height}"
        )
